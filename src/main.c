...
static SDL_HitTestResult HitTestCallback(SDL_Window *win, const SDL_Point *pt, void *data)
{
  ...
  {
    return (pt->x < RESIZE_BORDER) ? SDL_HITTEST_RESIZE_TOPLEFT : (pt->x >= w - RESIZE_BORDER) ? SDL_HITTEST_RESIZE_TOPRIGHT
                                                                                               : SDL_HITTEST_RESIZE_TOP;
  }
  else if (pt->y >= h - RESIZE_BORDER)
  {
    return (pt->x < RESIZE_BORDER) ? SDL_HITTEST_RESIZE_BOTTOMLEFT : (pt->x >= w - RESIZE_BORDER) ? SDL_HITTEST_RESIZE_BOTTOMRIGHT
...
{
  if (SDL_LockMutex(g_audio_mutex))
    Die("Mutex lock failed!");
  while (len != 0)
  {
...
static bool SdlRenderer_Init(SDL_Window *window)
{
  ...
  SDL_Renderer *renderer = SDL_CreateRenderer(g_window, -1,
                                              g_config.output_method == kOutputMethod_SDLSoftware ? SDL_RENDERER_SOFTWARE : SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
  if (renderer == NULL)
  {
...
#undef main // Global variables for the main loop
bool running = true;
...
void main_loop()
{
  while (SDL_PollEvent(&event))
  {
    ...
    case SDL_QUIT:
      running = false;
      emscripten_cancel_main_loop(); // Exit the main loop
      cleanup();
      return;
    }
  }
  ...
  {
    static const uint8 delays[3] = {16, 16, 17};
    uint32 tickDiff = curTick - lastTick;

    if (tickDiff < 40)
    {
      uint32 delay = tickDiff >= sizeof(delays) ? 16 : delays[tickDiff];
      SDL_Delay(delay);
    }
  }
...
static void HandleCommand_Locked(uint32 j, bool pressed)
{
  ...
  else
  {
    ...
    case kKeys_CheatWalkThroughWalls:
      PatchCommand('E');
      break;
    case kKeys_ClearKeyLog:
      PatchCommand('k');
      break;
    case kKeys_StopReplay:
      PatchCommand('l');
      break;
...
#endif
      break;
    case kKeys_ReplayTurbo:
      g_replay_turbo = !g_replay_turbo;
      break;
    ...
    case kKeys_VolumeDown:
      HandleVolumeAdjustment(j == kKeys_VolumeUp ? 1 : -1);
      break;
    default:
      assert(0);
    }
...
static int RemapSdlButton(int button)
{
  ...
  case SDL_CONTROLLER_BUTTON_B:
    return kGamepadBtn_B;
  case SDL_CONTROLLER_BUTTON_X:
    return kGamepadBtn_X;
  case SDL_CONTROLLER_BUTTON_Y:
    return kGamepadBtn_Y;
  case SDL_CONTROLLER_BUTTON_BACK:
    return kGamepadBtn_Back;
  case SDL_CONTROLLER_BUTTON_GUIDE:
    return kGamepadBtn_Guide;
  case SDL_CONTROLLER_BUTTON_START:
    return kGamepadBtn_Start;
  case SDL_CONTROLLER_BUTTON_LEFTSTICK:
    return kGamepadBtn_L3;
  case SDL_CONTROLLER_BUTTON_RIGHTSTICK:
    return kGamepadBtn_R3;
  case SDL_CONTROLLER_BUTTON_LEFTSHOULDER:
    return kGamepadBtn_L1;
  case SDL_CONTROLLER_BUTTON_RIGHTSHOULDER:
    return kGamepadBtn_R1;
  case SDL_CONTROLLER_BUTTON_DPAD_UP:
    return kGamepadBtn_DpadUp;
  case SDL_CONTROLLER_BUTTON_DPAD_DOWN:
    return kGamepadBtn_DpadDown;
  case SDL_CONTROLLER_BUTTON_DPAD_LEFT:
    return kGamepadBtn_DpadLeft;
  case SDL_CONTROLLER_BUTTON_DPAD_RIGHT:
    return kGamepadBtn_DpadRight;
  default:
    return -1;
  }
}
